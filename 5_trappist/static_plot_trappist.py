...
import numpy as np
from universe_variables import G, DAY, AU

#!!-------------------------------------------!!-------------------------------------------------!!
###############THIS FILE ONLY MAKES SMALL CHANGES COMPARED TO STATIC_PLOT_TOOLS.PY###############
#!!-------------------------------------------!!-------------------------------------------------!!

#Plots the variables given, but only every "plot_step" points.
def ax_plot_step(axis, plot_step, x, y, z = None, colour = None, marker=None):
    ...
    if z is not None:
        ...
        if colour is not None and marker is not None:
            return axis.plot(x, y, z, color=colour, marker=marker, linestyle='')
        return axis.plot(x, y, z)
    if colour is not None and marker is not None:
            return axis.plot(x, y, color=colour, marker=marker)
...
def Energies(t, x, vx, y, vy, z, vz, masses, ax_energy, body_count):
    ...
    for time_i in range (len(t)):
        ...
        for body_i in range(1,body_count):
            ...
            for body_j in range(body_i):
                ...
                if abs(r) <= 1e-6 :
                    raise Exception(f"Overlapping position for body {body_i} and body {body_j} at time {t[time_i]/DAY} years.")
    ...
    ax_plot_step(ax_energy, 1, t/DAY, (e-e[0])/e[0])
    
    print("Doing legend and titles...")
    ax_energy.set_xlabel("t (days)")
...
def static_plots(regPlot_3Dplot, regPlot_coordToTime, regPlot_energyToTime,
    ...
    if regPlot_3Dplot:
        ...
        for body in range(body_count):
            ...
            colours.append(ax_plot_step(ax_pos, plot_step, x[body]/AU, y[body]/AU, z[body]/AU)[0]._color)
        ...
        for body in range(body_count):
            if bodies_names:
                ...
                body_name = f"body {body}"
            ax_pos.scatter(x[body][0]/AU, y[body][0]/AU, z[body][0]/AU, marker='*', color=colours[body])
    ...
    if regPlot_coordToTime:
        ...
        for body in range(body_count):
            ...
            ax_plot_step(ax_coord_to_t, plot_step, t/DAY, coord[body]/AU)
            if bodies_names:
        ...
        print("Doing titles...")
        ax_coord_to_t.set_xlabel("t (days)")
        ax_coord_to_t.set_ylabel(f"{regPlot_coordToTime} (AU)")
    ...
    print("Doing legends")
    if regPlot_3Dplot:
        ax_pos.legend(legend_ax_pos, loc=(1.04,.2))
    if regPlot_coordToTime:
