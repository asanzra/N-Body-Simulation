...
import numpy as np
from universe_variables import AU, YEAR, G
...
def Energies(t, x, vx, y, vy, z, vz, masses, ax_energy, body_count):
    ...
    for time_i in range (len(t)):
        ...
        for body_i in range(1,body_count):
            ...
            for body_j in range(body_i):
                ...
                if abs(r) <= 1e-6 :
                    raise Exception(f"Overlapping position for body {body_i} and body {body_j} at time {t[time_i]/YEAR} years.")
    ...
    ax_plot_step(ax_energy, 1, t/YEAR, (e-e[0])/e[0])
    
    print("Doing legend and titles...")
    ax_energy.set_xlabel("t (years)")
...
def static_plots(regPlot_3Dplot, regPlot_coordToTime, regPlot_energyToTime,
    ...
    if regPlot_coordToTime:
        ...
        for body in range(body_count):
            ...
            ax_plot_step(ax_coord_to_t, plot_step, t/YEAR, coord[body]/AU)
            if bodies_names:
        ...
        print("Doing titles...")
        ax_coord_to_t.set_xlabel("t (years)")
